...
test("issue view fetches an explicit key and emits the primary issue record", async () => {
  ...
  const result = await runIre(["issue", "view", "ABC-123"], {
  ...
  assert.equal("comments" in envelope.data, false);
  assert.equal("pullRequests" in envelope.data, false);
...
test("issue view requires an explicit issue key", async () => {
  const hookPath = await writeFetchHook(`
  ...
  const result = await runIre(["issue", "view"], {
    nodeArgs: ["--import", hookPath],
    env: {
      IRE_JIRA_BASE_URL: "https://jira.example.test",
      IRE_JIRA_EMAIL: "agent@example.test",
...
test("issue view reports incomplete Jira configuration before network calls", async () => {
  const hookPath = await writeFetchHook(`
    globalThis.fetch = async () => {
      throw new Error("network call attempted");
    };
...
test("issue view omits absent optional fields and normalizes unset enrichment to nulls and empty lists", async () => {
  const hookPath = await writeFetchHook(`
  ...
  const result = await runIre(["issue", "view", "ABC-789"], {
    nodeArgs: ["--import", hookPath],
    env: {
      IRE_JIRA_BASE_URL: "https://jira.example.test",
      IRE_JIRA_EMAIL: "agent@example.test",
...
test("issue view maps provider 404 responses to a structured not-found error", async () => {
  const hookPath = await writeFetchHook(`
  ...
  const result = await runIre(["issue", "view", "ABC-404"], {
    nodeArgs: ["--import", hookPath],
    env: {
      IRE_JIRA_BASE_URL: "https://jira.example.test",
      IRE_JIRA_EMAIL: "agent@example.test",
...
test("issue view maps provider auth failures to exit code 3", async () => {
  const hookPath = await writeFetchHook(`
    globalThis.fetch = async () => Response.json(
  ...
  const result = await runIre(["issue", "view", "ABC-401"], {
    nodeArgs: ["--import", hookPath],
    env: {
      IRE_JIRA_BASE_URL: "https://jira.example.test",
      IRE_JIRA_EMAIL: "agent@example.test",
...
test("issue view debug metadata redacts authorization and records only the issue request", async () => {
  const hookPath = await writeFetchHook(`
    globalThis.fetch = async (input) => {
      const url = String(input);
...
test("issue view --comments adds every normalized comment and does not fetch pull requests", async () => {
  const hookPath = await writeFetchHook(`
    globalThis.fetch = async (input) => {
      const url = String(input);
  ...
  const result = await runIre(["issue", "view", "ABC-123", "--comments"], {
    nodeArgs: ["--import", hookPath],
    env: {
      IRE_JIRA_BASE_URL: "https://jira.example.test",
      IRE_JIRA_EMAIL: "agent@example.test",
...
test("issue view --pull-requests adds development-panel pull requests and does not fetch comments", async () => {
  const hookPath = await writeFetchHook(`
    globalThis.fetch = async (input) => {
      const url = String(input);
      ...
      if (url === "https://jira.example.test/rest/api/3/issue/ABC-123") {
        return Response.json({
          ...
          key: "ABC-123",
          fields: {
            summary: "Pull request expansion",
            status: { name: "In Progress" },
            issuetype: { name: "Bug" },
            project: { key: "ABC", name: "Agent Bridge" },
            labels: [],
  ...
  const result = await runIre(["issue", "view", "ABC-123", "--pull-requests"], {
    nodeArgs: ["--import", hookPath],
    env: {
      IRE_JIRA_BASE_URL: "https://jira.example.test",
      IRE_JIRA_EMAIL: "agent@example.test",
...
test("issue view --comments fails the whole command when comment pages fail", async () => {
  const hookPath = await writeFetchHook(`
    globalThis.fetch = async (input) => {
      const url = String(input);
      ...
      return Response.json({ message: "comments unavailable" }, { status: 503 });
    };
  `);
...
test("issue view --pull-requests fails the whole command when dev-status fails", async () => {
  const hookPath = await writeFetchHook(`
    globalThis.fetch = async (input) => {
      const url = String(input);
...
test("issue view --comments emits an empty comments array when the issue has none", async () => {
  const hookPath = await writeFetchHook(`
    globalThis.fetch = async (input) => {
      const url = String(input);
...
test("issue view accepts comments and pull-requests expansions together", async () => {
  const hookPath = await writeFetchHook(`
    globalThis.fetch = async (input) => {
      const url = String(input);
  ...
  const result = await runIre(
    ["issue", "view", "ABC-123", "--comments", "--pull-requests"],
    {
      nodeArgs: ["--import", hookPath],
      env: {
        IRE_JIRA_BASE_URL: "https://jira.example.test",
